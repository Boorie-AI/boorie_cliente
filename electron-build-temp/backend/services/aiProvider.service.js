...
class AIProviderService {
        ...
        try {
            ...
            if (testResult) {
                ...
                try {
                    ...
                    this.logger.warn('Error during auto-fetch models after connection test', error);
                }
            }
    ...
    async testAnthropic(provider) {
        try {
            ...
            try {
                ...
                this.logger.error('Anthropic API connection failed', fetchError);
    ...
    async fetchAPIModels(provider) {
        try {
            ...
            if (!provider.apiKey) {
                ...
                throw new models_1.AIProviderError('API key is required to fetch models', provider.name);
            }
            ...
            this.logger.error('Failed to fetch API models', error, { provider: provider.name });
            return [];
        }
    }
    ...
    async fetchAnthropicModels(provider) {
        try {
            ...
            this.logger.warn('Failed to fetch Anthropic models from API, falling back to known models', error);
            return this.getKnownAnthropicModels();
        }
    }
    getKnownAnthropicModels() {
    ...
    getAnthropicModelDescription(modelId) {
        const descriptions = {
    ...
    async fetchGoogleAIModels(provider) {
        try {
            const response = await fetch(`https://generativelanguage.googleapis.com/v1/models?key=${provider.apiKey}`);
            if (!response.ok) {
            ...
            this.logger.error('Failed to fetch Google AI models', error);
            return [];
        }
    }
