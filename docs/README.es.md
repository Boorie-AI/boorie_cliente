...
- En Configuración → Configuración de IA, arriba, se elige entre el automático, un modelo de este equipo (Ollama) o uno de un proveedor externo con clave. Con uno externo, la aplicación avisa de que su pregunta y los fragmentos de sus documentos salen del equipo.
- La búsqueda en sus documentos y la evaluación de las fuentes se siguen haciendo en local.
- Ver las [notas completas de la release](https://github.com/Boorie-AI/boorie_cliente/releases/tag/v1.39.0).
...
- El aviso de progreso ya no dice «0 de 0 fragmentos» mientras todavía los está contando.
- El asistente decía ser GPT-4 corriendo en Ollama: un modelo no sabe cómo se llama y, si se le pide decirlo, se lo inventa. Ahora la aplicación le dice con qué modelo redacta y con qué modelo de embeddings se buscaron las fuentes.
- Ver las [notas completas de la release](https://github.com/Boorie-AI/boorie_cliente/releases/tag/v1.38.1).
...
- Ahora lo legible funciona con normalidad y lo ilegible se nombra, para que usted sepa cuál borrar.
- Ver las [notas completas de la release](https://github.com/Boorie-AI/boorie_cliente/releases/tag/v1.37.1).

### 📝 Novedades en v1.37.0
- El asistente respondía preguntas que no son suyas inventándose la respuesta: preguntado cuántas hamburguesas salen de un kilo de carne, contestaba «40». Ahora avisa de que eso queda fuera de lo que sabe, al instante, y obedece a los interruptores de Configuración → Guardrails.
- Un PDF del que no se podía sacar texto se indexaba igual, con una frase inventada dentro. Ahora no se indexa y se dice por qué; si es un escaneado, hay que pasarle OCR. Los que ya estaban así se detectan y se nombran.
- La base de conocimiento venía apagada sin advertirlo, así que usted subía documentos y el asistente respondía sin mirarlos. Ahora arranca encendida cuando hay documentos indexados, y si usted la apaga se queda apagada.
- Ver las [notas completas de la release](https://github.com/Boorie-AI/boorie_cliente/releases/tag/v1.37.0).
...
- Recorrerlos no cambiaba nada: las tuberías no daban caudal ni velocidad, los nudos no daban presión y repetían la demanda base del fichero en todos los pasos, porque detrás no había ninguna simulación.
- La barra sólo aparece ahora cuando hay resultados de verdad que recorrer.
- Ver las [notas completas de la release](https://github.com/Boorie-AI/boorie_cliente/releases/tag/v1.35.0).
...
- Ahora el periodo se elige una sola vez y viene dicho al lado de la cifra: 1 día, 3 días, 1 semana o la duración del fichero de la red, que es la opción de partida. Sustituye al campo «Duración simulación (h)».
- El análisis energético medía siempre el primer día, durase lo que durase la red. Pasa al mismo periodo que el resto, y dice cuál ha usado.
- Los embalses aparecían como los nudos más críticos de la red: su presión es cero por definición, así que desplazaban a los nudos de consumo que sí tenían un problema real.
- «Nivel de servicio» aparecía siempre vacío, con cualquier red. Ya se calcula, con el mismo criterio que el panel de indicadores.
- Ver las [notas completas de la release](https://github.com/Boorie-AI/boorie_cliente/releases/tag/v1.33.0).

### 📝 Novedades en v1.32.0
- Abrir el centro de conocimiento con muchos documentos se llevaba por delante la aplicación. Para escribir «90 fragmentos» al lado de cada título se cargaba en memoria el vector de cada fragmento —unos 29 KB cada uno— y los contaba ahí: en una base de 1,1 GB eso es pedir cerca de un gigabyte cada vez que se abre el panel, y otra vez después de cada subida. Ahora esas cuentas las hace la base de datos.
- Un documento podía quedar indexado sobre el papel y ausente de la búsqueda. La base vectorial se creaba siempre con el tamaño del modelo local, así que lo indexado con OpenAI se rechazaba en cada inserción sin decir nada, y la búsqueda semántica no lo encontraba nunca.
- El desplegable de modelos de indexación ya no ofrece opciones que no pueden funcionar —OpenAI sin clave, o modelos de Ollama que no están instalados—, y cuando no hay ninguno disponible explica qué instalar.
- El contador de la carga por carpeta deja de enseñar números que no corresponden: contaba los documentos que salían bien en vez de por cuál iba.
- Ver las [notas completas de la release](https://github.com/Boorie-AI/boorie_cliente/releases/tag/v1.32.0).
...
- La calculadora ofrece l/s por defecto en los campos de caudal y devuelve en l/s el resultado de la ecuación de continuidad y del flujo por orificio. No se pierde ninguna opción, y la conversión aparece como un paso más del cálculo.
- El nivel de servicio por presión se enseña en porcentaje en los dos sitios donde aparece; salía como «98,1 %» en uno y «0.9812» en el otro.
- Ver las [notas completas de la release](https://github.com/Boorie-AI/boorie_cliente/releases/tag/v1.27.0).
...
- Si preparar la red falla, ahora se dice: antes no había forma de saber que podías verla pero no simularla.
- Mientras simulaba se dibujaban dos barras de progreso en lugar de una.
- Corregidas las instrucciones de instalación de este README, que mandaban descargar la versión 1.15.0.
- Ver las [notas completas de la release](https://github.com/Boorie-AI/boorie_cliente/releases/tag/v1.21.1).
