...
- La primera vegada que vostè obri aquesta versió, la base vectorial es reorganitza sola, una vegada, copiant els vectors que ja té: en aquesta base són uns 11 minuts. La Base de Coneixement diu quant porta, i si tanca l'aplicació continua per on anava.
- El model d'embeddings per defecte passa a ser `granite-embedding:278m`: recupera millor i reindexa tres vegades més de pressa. Cal reindexar una vegada, i si falta el model, l'aplicació el demana i el descarrega.
- I una sèrie d'arranjaments pels quals el RAG no trobava res sense dir-ho: la comprovació de salut mirava un sol fragment, al model local no li arribava el prompt de sistema, i el troceig deixava la major part de cada fragment fora de la cerca.
- Vegeu les [notes completes de la release](https://github.com/Boorie-AI/boorie_cliente/releases/tag/v1.38.0).
...
- I respon en l'idioma que vostè té posat a l'aplicació, no en el dels documents. Quan cita un text en un altre idioma, ho diu.
- Els informes que Boorie escriu en simular tapaven la documentació a les cerques, i se n'acumulava un de nou per cada execució amb el mateix nom que l'anterior. Ara cada cosa té el seu lloc i l'informe se substitueix en comptes d'apilar-se.
- El centre de coneixement mostra ja els resultats de la cerca per significat, que abans es demanaven i es llençaven.
- En actualitzar cal reindexar la documentació una sola vegada: el centre de coneixement ho avisa i porta el botó. No es fa sol.
- Vegeu les [notes completes de la release](https://github.com/Boorie-AI/boorie_cliente/releases/tag/v1.36.0).
...
- Ara, quan ningú demana un període concret, s'utilitza el que declara el fitxer de la xarxa.
- I el text de l'assistent deia «al dia» sobre xifres que no eren d'un dia —el consum, l'estalvi de cada mesura i el motiu pel qual es proposava—, així que la frase contradeia el número que tenia al costat. Les tres diuen ara el període que es va simular.
- Vegeu les [notes completes de la release](https://github.com/Boorie-AI/boorie_cliente/releases/tag/v1.34.0).
...
- L'avís viatja dins del que s'exporta: un CSV, un informe o una conversa es reenvien per correu sense l'aplicació al voltant, i ningú no torna a la pantalla original a llegir l'advertiment.
- Els fitxers exportats es deien com si fossin una altra cosa. El de la corba de fragilitat sortia com «curva_fragilidad_PVC_Net3 2.inp.csv», i a Windows —que amaga les extensions conegudes— es llegia «…Net3 2.inp». Ara el nom no arrossega l'extensió de la xarxa.
- Vegeu les [notes completes de la release](https://github.com/Boorie-AI/boorie_cliente/releases/tag/v1.29.0).
...
- Si la qualitat no es pot simular, es diu, en lloc de mostrar un número. Les altres dues simulacions del cicle es desen igualment.
- Es pot triar què seguir: l'edat de l'aigua, un traçador des de l'embassament o el dipòsit que triïs, o la substància que declari el teu fitxer.
- Les xifres porten la seva unitat: hores, per cent o mg/L segons què se segueixi.
- Les teves simulacions de qualitat anteriors continuen allà, marcades com a «sin simular» a l'historial de la xarxa. No s'esborra res.
- Vegeu les [notes completes de la release](https://github.com/Boorie-AI/boorie_cliente/releases/tag/v1.24.0).

### 📝 Novetats a v1.23.1
- Després d'aturar la reproducció, el botó de reproduir no la tornava a engegar: es premia «stop», la barra tornava al primer pas i a partir d'aquí «play» ja no feia res, calia tornar a carregar la xarxa.
- Li passava el mateix, encara que costava més adonar-se'n, després d'anar al principi, anar al final o arrossegar la barra amb el ratolí: qualsevol manera de moure el pas a mà deixava la reproducció morta.
- Vegeu les [notes completes de la release](https://github.com/Boorie-AI/boorie_cliente/releases/tag/v1.23.1).
